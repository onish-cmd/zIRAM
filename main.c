...
static uint32_t calc_entropy(const unsigned char *page) {
    ...
    // Histogram calculation with unrolled loop for speed on AMD E-350
    for (i = 0; i < 4096; i += 8) {
    ...
    // Return shifted result (scaled by 1024)
...
static int zram_write_page(struct zram *zram, struct page *page, u32 index)
{
    ...
    bool same_filled;
    uint32_t entropy; // zIRAM variable

    // 1. Map the page to check if it's full of the same character (e.g., all zeros)
    mem = kmap_local_page(page);
    
    // --- zIRAM HIJACK START ---
    entropy = calc_entropy(mem);
    
    // If entropy is High dont waste cpu cycles
    if (entropy > (7 << 10)) {
        kunmap_local(mem);
        pr_info_ratelimited("zIRAM: High Entropy (%u) -> BYPASS\n", entropy);
